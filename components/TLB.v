...
module TLB #(
    ...
    reg  [      32-13-1:0] tlb_vppn                              [TLB_ENTRIES-1:0];
    reg  [            5:0] tlb_ps                                [TLB_ENTRIES-1:0];
    reg                    tlb_g                                 [TLB_ENTRIES-1:0];
    reg  [            9:0] tlb_asid                              [TLB_ENTRIES-1:0];
    reg  [TLB_ENTRIES-1:0] tlb_e;
    reg  [      32-12-1:0] tlb_ppn0                              [TLB_ENTRIES-1:0];
    reg  [            1:0] tlb_plv0                              [TLB_ENTRIES-1:0];
    reg  [            1:0] tlb_mat0                              [TLB_ENTRIES-1:0];
    reg                    tlb_d0                                [TLB_ENTRIES-1:0];
    reg                    tlb_v0                                [TLB_ENTRIES-1:0];
    reg  [      32-12-1:0] tlb_ppn1                              [TLB_ENTRIES-1:0];
    reg  [            1:0] tlb_plv1                              [TLB_ENTRIES-1:0];
    reg  [            1:0] tlb_mat1                              [TLB_ENTRIES-1:0];
    reg                    tlb_d1                                [TLB_ENTRIES-1:0];
    reg                    tlb_v1                                [TLB_ENTRIES-1:0];

    wire [           31:0] tlb_va                                [TLB_ENTRIES-1:0];
    wire [           31:0] invtlb_va = {invtlb_vpn, 13'h0};
    wire [           31:0] invtlb_vpn_match                      [TLB_ENTRIES-1:0];
    wire [TLB_ENTRIES-1:0] invtlb_match;
    wire [           31:0] s0_va = {s0_vppn, s0_va_bit12, 12'h0};
    wire [           31:0] s0_vppn_match                         [TLB_ENTRIES-1:0];
    wire [TLB_ENTRIES-1:0] s0_match;
    wire [           31:0] s1_va = {s1_vppn, s1_va_bit12, 12'h0};
    wire [           31:0] s1_vppn_match                         [TLB_ENTRIES-1:0];
    wire [TLB_ENTRIES-1:0] s1_match;
    ...
    generate
        for (i = 0; i < TLB_ENTRIES; i = i + 1) begin : gen_tlb
            ...
            assign invtlb_vpn_match[i][31] = 1'b1;
            assign s0_vppn_match[i][31]    = 1'b1;
            assign s1_vppn_match[i][31]    = 1'b1;
            for (j = 0; j < 31; j = j + 1) begin : gen_invtlb_vpn_match
                assign invtlb_vpn_match[i][j] = tlb_va[i][31:j+1] == invtlb_va[31:j+1];
                assign s0_vppn_match[i][j]    = tlb_va[i][31:j+1] == s0_va[31:j+1];
                assign s1_vppn_match[i][j]    = tlb_va[i][31:j+1] == s1_va[31:j+1];
            end
