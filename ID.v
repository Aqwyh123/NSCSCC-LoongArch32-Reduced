...
module ID (
    ...
    assign GPR_read_src2_is_rd = st_b | st_h | st_w |
                                 beq | bne | blt | bge | bltu | bgeu | csrwr | csrxchg;

    assign ALU_src1_is_PC = jirl | bl | pcaddu12i;
    assign ALU_src2_is_imm = slli_w | srli_w | srai_w | slti | sltui |
                             addi_w | andi | ori | xori | lu12i_w | pcaddu12i |
                             ld_b | ld_h | ld_w | ld_bu | ld_hu | st_b | st_h | st_w |
                             jirl | bl | cacop;

    assign ALU_operation[`ALU_OP_ADD] = add_w | addi_w | jirl | bl | pcaddu12i | cacop |
                                        ld_b | ld_h | ld_w | st_b | st_h | st_w | ld_bu | ld_hu;
    ...
    assign cache_operation[`CACHE_OP_INDEX_OP] = cacop & instruction[4:3] == `CACOP_TYPE_INDEX_OP;
    assign cache_operation[`CACHE_OP_HIT_OP] = cacop & instruction[4:3] == `CACOP_TYPE_HIT_OP;
    ...
    assign refetch =  tlbrd | tlbwr | tlbfill | invtlb |
                     (csrwr | csrxchg) & (instruction[`I14_MSB:`I14_LSB] == `CSR_CRMD |
                      ...
                      cacop & instruction[2:0] ==`CACOP_TARGET_ICACHE;
    ...
    assign GPR_new[`GPR_NEW_EXE] = jirl | bl | lu12i_w | csrrd | csrwr | csrxchg |
                                   rdcntid_w | rdcntvl_w | rdcntvh_w;
