...
module MEM_reg (
    ...
    input  wire [`GPR_WRITE_WIDTH-1:0] EXE_GPR_write,
    input  wire [                 4:0] EXE_GPR_write_num,
    input  wire [                31:0] EXE_GPR_write_data,
    output reg  [                31:0] MEM_PC,
    output reg  [ `MEM_READ_WIDTH-1:0] MEM_MEM_read,
    output reg  [                31:0] MEM_MEM_addr,
    output reg  [                31:0] MEM_ALU_result,
    output reg  [`GPR_WRITE_WIDTH-1:0] MEM_GPR_write,
    output reg  [                 4:0] MEM_GPR_write_num,
    output reg  [                31:0] MEM_EXE_GPR_write_data
);
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            MEM_GPR_write          <= `GPR_WRITE_WIDTH'b0;
            MEM_GPR_write_num      <= 5'b0;
            MEM_EXE_GPR_write_data <= 32'h0;
        end else begin
            if (MEM_ready) begin
            ...
            if (EXE_to_MEM_valid & MEM_ready) begin
                ...
                MEM_EXE_GPR_write_data <= EXE_GPR_write_data;
            end
        end
    end
endmodule
