...
module EXE_stage (
    ...
    assign done = |exception ? 1'b1 :
                  |MEM_read | |{MEM_write[`MEM_WRITE_COND] & llbit,
                                MEM_write[`MEM_WRITE_WORD:`MEM_WRITE_BYTE]} ? MEM_done : ALU_done;
    ...
    assign data_load = |MEM_read | cache_operation[`CACHE_OP_HIT];
    assign data_store = |{MEM_write[`MEM_WRITE_COND] & llbit,
                          MEM_write[`MEM_WRITE_WORD:`MEM_WRITE_BYTE]};
    assign data_vaddr = ALU_result;

    assign data_sram_req = valid & (|MEM_read | |{MEM_write[`MEM_WRITE_COND] & llbit,
                                                  MEM_write[`MEM_WRITE_WORD:`MEM_WRITE_BYTE]}) &
                                   ~|exception & MEM_ready;
    ...
    assign data_sram_size = {2{MEM_read[`MEM_READ_BYTE] | MEM_read[`MEM_READ_BYTEU] |
                            ...
                            {2{|MEM_read[`MEM_READ_LINK:`MEM_READ_WORD] |
                               |MEM_write[`MEM_WRITE_COND:`MEM_WRITE_WORD]}} & 2'b10;
    ...
    assign ALE = (MEM_read[`MEM_READ_HALF] | MEM_read[`MEM_READ_HALFU] | MEM_write[`MEM_WRITE_HALF])
                 & ALU_result[0] |
                 (|MEM_read[`MEM_READ_LINK:`MEM_READ_WORD] |
                  |{MEM_write[`MEM_WRITE_COND] & llbit, MEM_write[`MEM_WRITE_WORD]})
                 & |ALU_result[1:0];
endmodule
